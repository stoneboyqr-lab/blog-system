...
(function () {
  ...
  function updateStats(filtered) {
    ...
    if (statPublished) statPublished.firstChild.nodeValue = String(filtered.length);
    if (statCategories) statCategories.firstChild.nodeValue = String(allCategories.length);
    if (statWords) {
      const words = filtered.reduce((sum, p) => sum + stripHtml(p.content).split(/\s+/).filter(Boolean).length, 0);
      statWords.firstChild.nodeValue = String(Math.max(0, Math.round(words / 1000)));
    }
  }
  ...
  function applyFilter() {
    ...
    if (featuredRow) {
      featuredRow.innerHTML = featuredPosts.length ? featuredPosts.map(featuredCard).join("") : "";
    }
